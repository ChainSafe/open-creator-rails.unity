...
namespace Io.ChainSafe.OpenCreatorRails
{
    public class Asset : MonoBehaviour
    {
        ...
        {
            AssetDto assetDto = await OpenCreatorRailsService.Instance.IndexerProvider.GetAsset(AssetIdHash, _registryAddress);
            ...
            // TODO react to contract events to update data
        ...
        public async UniTask<DateTime> Subscribe(string subscriberId, TimeSpan duration)
        {
            ...
            EthECDSASignature signature = OpenCreatorRailsService.Instance.WalletProvider.SignTypedData(permit, typedData);

            byte[] subscriberHashBytes = subscriberId.Keccack256Bytes();
            
            TransactionReceipt receipt = await Service.SubscribeRequestAndWaitForReceiptAsync(subscriberHashBytes, permit.Owner, permit.Spender, permit.Value, permit.Deadline, signature.V[0], signature.R, signature.S);
